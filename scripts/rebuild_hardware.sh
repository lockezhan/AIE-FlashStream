...
[[ "$BUILD_DIR" == "$ROOT/build.hw" ]] || exit 2
rm -rf -- "$BUILD_DIR"
mkdir -p "$BUILD_DIR"
...
(cd "$ROOT/aie" && aiecompiler --target=hw --platform="$PLATFORM" \
  ...
  --workdir="$BUILD_DIR/work_aie" --output-archive="$BUILD_DIR/libadf.a" \
...
(cd "$ROOT" && v++ -c -t hw --platform "$PLATFORM" --kernel_frequency 300 \
  ...
  --report_dir "$BUILD_DIR/reports_pl" -o "$BUILD_DIR/llama3_attention.xo" \
...
(cd "$ROOT" && v++ -l -t hw --platform "$PLATFORM" --optimize 2 \
  ...
  --report_dir "$BUILD_DIR/reports_link" --config link/eight_groups_packet.cfg \
  -o "$BUILD_DIR/llama3_attention.xsa" \
  "$BUILD_DIR/llama3_attention.xo" "$BUILD_DIR/libadf.a") \
...
v++ --package -t hw --platform "$PLATFORM" --temp_dir "$BUILD_DIR/_x_package" \
  ...
  -o "$BUILD_DIR/llama3_attention.xclbin" \
  "$BUILD_DIR/llama3_attention.xsa" "$BUILD_DIR/libadf.a" \
  2>&1 | tee "$BUILD_DIR/vpp_package.log"
sha256sum "$BUILD_DIR/libadf.a" "$BUILD_DIR/llama3_attention.xo" \
  "$BUILD_DIR/llama3_attention.xsa" \
  "$BUILD_DIR/llama3_attention.xclbin" | tee "$BUILD_DIR/artifact_sha256.txt"
