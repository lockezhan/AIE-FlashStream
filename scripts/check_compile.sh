...
[[ "$BUILD_DIR" == "$ROOT/build.check" ]] || exit 2
rm -rf -- "$BUILD_DIR"
mkdir -p "$BUILD_DIR"
...
python3 "$ROOT/tests/aie_sim.py" generate --dir "$ROOT/aie"
(cd "$ROOT/aie" && aiecompiler --target=x86sim --platform="$PLATFORM" \
  --include="$ROOT/aie" --include="$XILINX_VITIS_AIETOOLS/include" \
  --workdir="$BUILD_DIR/work_x86sim" --output-archive="$BUILD_DIR/libadf_x86.a" \
  llama3_gqa_8group_packet_sim.cpp) 2>&1 | tee "$BUILD_DIR/aiecompiler_x86.log"
(cd "$ROOT/aie" && x86simulator --pkg-dir="$BUILD_DIR/work_x86sim") 2>&1 | tee "$BUILD_DIR/x86sim.log"
python3 "$ROOT/tests/aie_sim.py" verify --dir "$ROOT/aie"
...
echo "AIE-FlashStream compile check PASS"
