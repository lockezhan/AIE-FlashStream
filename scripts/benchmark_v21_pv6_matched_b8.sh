...
RESULT_DIR="${2:-$ROOT/results/matched_b8_pure_pl_v1_vs_v21_pv6_20260830}"
SAMPLES="${3:-10}"
...
if [[ "$xclbin_sha" != "$EXPECTED_XCLBIN_SHA" ]]; then
  echo "refusing non-V21 XCLBIN: $xclbin_sha" >&2
  exit 3
fi

{
  ...
  printf 'device_bdf=%s\n' "$BDF"
  printf 'workload=B8,S32,HQ32,HKV8,D128\n'
...
for sample in $(seq 1 "$SAMPLES"); do
  ...
  sleep 2
  echo "[$tag] running one independent B8 launch"
  timeout 120s "$HOST" --xclbin "$XCLBIN" --batch 8 --warmup 0 --runs 1 \
...
echo "Matched V21 B8 evidence: $RESULT_DIR"
