...
#ifdef __X86SIM__
#define LLAMA3_SCORE_TILE_LOCAL thread_local static
#else
#define LLAMA3_SCORE_TILE_LOCAL static
#endif
  alignas(aie::vector_decl_align) LLAMA3_SCORE_TILE_LOCAL int8 chess_storage(DM_bankA)
      k_cache_lo[kKeyOctets * kGroupOctets * kMmulK * kMmulCols];
  alignas(aie::vector_decl_align) LLAMA3_SCORE_TILE_LOCAL int8 chess_storage(DM_bankC)
      k_cache_hi[kKeyOctets * kGroupOctets * kMmulK * kMmulCols];
  alignas(aie::vector_decl_align) LLAMA3_SCORE_TILE_LOCAL float
      k_scales[kKeys * kGroups];
  LLAMA3_SCORE_TILE_LOCAL uint8 phase = 0;
#undef LLAMA3_SCORE_TILE_LOCAL
...
#ifdef __X86SIM__
  std::printf("V20_SCORE_CACHE instance=%p phase=%u action=%s k0=%.9g\n",
              static_cast<void*>(&phase), static_cast<unsigned>(phase),
              phase == 0 ? "load" : "reuse-no-read", k_scales[0]);
#endif

  // One graph invocation is one 2Q transaction.  Phase 0 consumes the
  // resident K packet above; phase 1 deliberately reads no K tokens.
  for (int query = 0; query < llama3_detail::kQueriesPerPhase; ++query) {
...
#pragma chess_prepare_for_pipelining
          ...
          score_lo =
              aie::mul(score_lo, k_scale_lo).template to_vector<float>();
          score_lo =
              aie::mul(score_lo, q_scale_lo).template to_vector<float>();
          auto score_hi = aie::to_float(
              accumulator_hi.template to_vector<int32>(0));
          score_hi =
              aie::mul(score_hi, k_scale_hi).template to_vector<float>();
          score_hi =
              aie::mul(score_hi, q_scale_hi).template to_vector<float>();
        dequantized = aie::add(score_lo, score_hi);
      }

#ifdef __X86SIM__
      if (row_pair == 1 && key_octet == 0) {
        alignas(aie::vector_decl_align) float trace_scores[16];
        aie::store_v(trace_scores, dequantized);
        std::printf(
            "V20_SCORE_TRACE instance=%p phase=%u query=%d q0=%.9g "
            "row3=%.9g,%.9g,%.9g,%.9g\n",
            static_cast<void*>(&phase), static_cast<unsigned>(phase), query,
            q_scales[0], trace_scores[8], trace_scores[9], trace_scores[10],
            trace_scores[11]);
      }
#endif
